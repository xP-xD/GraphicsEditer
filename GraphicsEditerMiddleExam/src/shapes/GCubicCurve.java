...
public class GCubicCurve extends GShape{
	...
	public void resizePoint(int x, int y) {
		...
		if(this.xPoint.size() > 2){
			cubicCurve2D.setCurve(this.xPoint.get(0) , this.yPoint.get(0) , x, y, this.xPoint.get(2), this.yPoint.get(2), this.xPoint.get(1), this.yPoint.get(1));
		}else if(this.xPoint.size() > 1) {
			cubicCurve2D.setCurve(this.xPoint.get(0) , this.yPoint.get(0), x, y, x, y, this.xPoint.get(1), this.yPoint.get(1));
		}else {
	...
	public void movePoint(int x, int y) {
		CubicCurve2D cubicCurve2D = (CubicCurve2D)shape;
		cubicCurve2D.setCurve(cubicCurve2D.getX1()+x-px,cubicCurve2D.getY1()+x-px,cubicCurve2D.getCtrlX1(),cubicCurve2D.getCtrlY1(),cubicCurve2D.getCtrlX2(), cubicCurve2D.getCtrlY2(), cubicCurve2D.getX2()+x-px, cubicCurve2D.getY2()+y-py);
		this.px = x;
		this.py = y;
	}
